...
public sealed class SlurmService : ISlurmService
{
    ...
    public SlurmService(ISshClientService ssh)
    {
    ...
    public async Task<SlurmJobStatus?> GetJobStatusAsync(long jobId, CancellationToken ct = default)
    {
        var (stdout, _, _) = await _ssh.ExecuteAsync(
            $"squeue --job {jobId} --noheader --format=\"%i|%j|%u|%T|%P|%D|%C|%N|%M|%S\"", ct);

        var line = stdout.Trim();
        if (string.IsNullOrEmpty(line)) return null;

        return ParseSqueueLine(line);
    }
    ...
    {
        var (stdout, _, _) = await _ssh.ExecuteAsync(
            $"squeue --user {username} --noheader --format=\"%i|%j|%u|%T|%P|%D|%C|%N|%M|%S\"", ct);

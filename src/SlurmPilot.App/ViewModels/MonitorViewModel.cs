...
using SlurmPilot.Infrastructure.Resilience;
...
public sealed class MonitorViewModel : ViewModelBase, IDisposable
{
    ...
    public string ToggleAllUsersButtonText => ShowAllUsers
        ? L("Monitor.StopViewAll")
        : L("Monitor.ViewAll");

    public string ToggleAllUsersButtonStyleKey => ShowAllUsers
        ? "AccentButtonStyle"
        : "NeutralButtonStyle";
    ...
    public bool ShowAllUsers
    {
        ...
        set
        {
            if (SetField(ref _showAllUsers, value))
            {
                ...
                OnPropertyChanged(nameof(CanQueryHistory));
                OnPropertyChanged(nameof(MonitorContextSummary));
                OnPropertyChanged(nameof(ToggleAllUsersButtonText));
                OnPropertyChanged(nameof(ToggleAllUsersButtonStyleKey));
                CommandManager.InvalidateRequerySuggested();
                if (value)
                    StartPolling();
    ...
    public ICommand ToggleAllUsersCommand { get; }
        ...
        INotificationService? notificationService = null)
    {
        ...
        ToggleAllUsersCommand = new RelayCommand(() => ShowAllUsers = !ShowAllUsers);
    ...
    private void ShowHistoryDetail(JobRow? row)
    {
        ...
        var detailLines = new List<string>
        {
            ...
            $"{L("Monitor.ColNodes")}: {ValueOrDash(row.NodeDisplay)}",
    ...
    {
        var mode = _prefs?.JobMonitorNotificationMode ?? JobMonitorNotificationMode.AppWidget;
        if (mode == JobMonitorNotificationMode.None)
            return;
...
public sealed class JobRow : ViewModelBase
{
    ...
    public string NodeList
    {
        ...
        set
        {
            if (SetField(ref _nodeList, value))
                OnPropertyChanged(nameof(NodeDisplay));
        }
    }
    ...
    public int NumNodes
    {
        ...
        set
        {
            if (SetField(ref _numNodes, Math.Max(0, value)))
                OnPropertyChanged(nameof(NodeDisplay));
        }
    }
    ...
    public string NodeDisplay
    {
        get
        {
            var nodeList = string.IsNullOrWhiteSpace(NodeList) ? string.Empty : NodeList.Trim();
            if (NumNodes <= 0)
                return nodeList;

            return string.IsNullOrWhiteSpace(nodeList)
                ? $"/ {NumNodes}"
                : $"{nodeList} / {NumNodes}";
        }
    }

...
public sealed class TaskBlueprintCreateViewModel : ViewModelBase
{
    ...
    {
        var legacyProgram = record?.TaskUnits
            .SelectMany(x => x.ProgramEntries)
            .Select(x => x.ProgramPath?.Trim())
            .FirstOrDefault(x => !string.IsNullOrWhiteSpace(x));
        if (!string.IsNullOrWhiteSpace(legacyProgram))
            return legacyProgram;

        return record?.TaskUnits
            .SelectMany(x => x.CommandEntries)
            .Select(command =>
            {
                var programPath = command.ProgramPath?.Trim();
                if (!string.IsNullOrWhiteSpace(programPath))
                    return programPath;

                var pythonPath = command.PythonInterpreterPath?.Trim();
                return command.UsePythonInterpreter && !string.IsNullOrWhiteSpace(pythonPath)
                    ? $"Python: {pythonPath}"
                    : null;
            })
            .FirstOrDefault(x => !string.IsNullOrWhiteSpace(x));

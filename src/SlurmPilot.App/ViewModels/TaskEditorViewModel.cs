...
public sealed class TaskEditorViewModel : ViewModelBase, IDisposable
{
    ...
    private void AttachCommandTracking(CommandEntryViewModel command)
    {
        ...
        command.EnvironmentVariables.CollectionChanged += OnEnvironmentVariablesCollectionChanged;
        ...
        foreach (var env in command.EnvironmentVariables)
            env.PropertyChanged += OnTrackedPropertyChanged;
    }
    ...
    private void OnEnvironmentVariablesCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        if (e.NewItems != null)
        {
            foreach (var item in e.NewItems.OfType<EnvironmentVariableViewModel>())
                item.PropertyChanged += OnTrackedPropertyChanged;
        }

        if (e.OldItems != null)
        {
            foreach (var item in e.OldItems.OfType<EnvironmentVariableViewModel>())
                item.PropertyChanged -= OnTrackedPropertyChanged;
        }
        ...
        var taskUnits = TaskUnits.Select(unit => new
        {
            ...
            SbatchOptions = unit.SbatchOptions is null
                ...
                : new
                {
                    ...
                    unit.SbatchOptions.TaskCount,
                    unit.SbatchOptions.CpuCount,
                    unit.SbatchOptions.GpuCount,
            ...
            Commands = unit.Commands.Select(command => new
            {
                ...
                EnvironmentVariables = command.EnvironmentVariables
                    .Select(env => new { env.Key, env.Value })
                    .ToList(),
            }).ToList(),
    ...
    private static TaskUnit CloneTaskUnit(TaskUnit source) => new()
    {
        ...
        CommandEntries = source.CommandEntries.Select(c => new CommandEntry
        {
            ...
            EnvironmentVariables = c.EnvironmentVariables
                .Select(env => new EnvironmentVariableEntry { Key = env.Key, Value = env.Value })
                .ToList(),
            PythonInterpreterPath = c.PythonInterpreterPath,
            UsePythonInterpreter = c.UsePythonInterpreter || !string.IsNullOrWhiteSpace(c.PythonInterpreterPath),
        ...
        var commands = unit == null
            ...
            : unit.Commands
                .Select((command, index) => new CommandValidationItem
                {
                    ...
                    PythonInterpreterPath = command.PythonInterpreterPath ?? string.Empty,

...
public sealed class DesktopJobNotificationService
{
        ...
        Action? clickAction)
    {
        ...
        dispatcher.InvokeAsync(() =>
        {
            ...
            var window = new JobNotificationWindow(title, message, type, duration, clickAction, CloseAll, _activeWindows.Count, _bottomReservedHeight);
            window.Closed += (_, _) =>
            {
    ...
    public void SetBottomReservedHeight(double height)
    {
        var dispatcher = Application.Current?.Dispatcher;
        if (dispatcher == null)
            return;
        ...
        dispatcher.InvokeAsync(() =>
        {
            _bottomReservedHeight = Math.Max(0, height);
            ReflowActiveWindows();
        });
    }
    ...
    private void ReflowActiveWindows()
    {
        ...
        for (var index = 0; index < _activeWindows.Count; index++)
            _activeWindows[index].MoveToStackIndex(index, animate: true, _bottomReservedHeight);
    }
}

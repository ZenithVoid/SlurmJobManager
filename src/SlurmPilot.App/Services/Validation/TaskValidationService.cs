...
public sealed class TaskValidationService : ITaskValidationService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var commands = context.Commands
            .Select((command, index) => new CommandValidationItem
            {
                ...
                PythonInterpreterPath = NormalizeRemotePath(command.PythonInterpreterPath),
                UsePythonInterpreter = command.UsePythonInterpreter || !string.IsNullOrWhiteSpace(command.PythonInterpreterPath),
        ...
        if (!string.IsNullOrWhiteSpace(sbatchOptions.GpuCount) && !IsPositiveInteger(sbatchOptions.GpuCount))
        {
            issues.Add(CreateIssue(
                "sbatch.gpu.invalid",
                "Task.Validation.GpuInvalidTitle",
                string.Format(L("Task.Validation.GpuInvalidDetail"), sbatchOptions.GpuCount ?? string.Empty),
                TaskValidationSeverity.Error,
                isBlocking: true,
                detailIsResourceKey: false));
        }
        ...
        if (!IsSbatchOptionSynchronized(sbatchTemplate, "--job-name", sbatchOptions.JobName)
            ...
            || !IsOptionalSbatchGresSynchronized(sbatchTemplate, sbatchOptions.GpuCount)
        ...
        if (parameterFiles.Count == 0 && !commands.Any(HasCommandPayload))
        {
            issues.Add(CreateIssue(
                "param.required",
        ...
        CancellationToken ct)
    {
        foreach (var command in commands.Where(HasCommandPayload))
        {
            ...
            var usesPython = command.UsePythonInterpreter || !string.IsNullOrWhiteSpace(command.PythonInterpreterPath);
            var hasStructuredFields = usesPython || !string.IsNullOrWhiteSpace(command.MpirunPath) || command.ParameterFiles.Count > 0;

            if (usesPython && string.IsNullOrWhiteSpace(command.PythonInterpreterPath))
            {
                issues.Add(CreateIssue(
                    "command.python.required",
                    "Task.Validation.CommandPythonRequiredTitle",
                    string.Format(L("Task.Validation.CommandPythonRequiredDetail"), order),
                    TaskValidationSeverity.Error,
                    isBlocking: true,
                    detailIsResourceKey: false));
            ...
            {
                var pythonAvailable = await IsExecutableReferenceAvailableAsync(command.PythonInterpreterPath, ct);
                if (!pythonAvailable)
                {
                    issues.Add(CreateIssue(
                        "command.python.unavailable",
                        "Task.Validation.CommandPythonUnavailableTitle",
                        string.Format(L("Task.Validation.CommandPythonUnavailableDetail"), order, command.PythonInterpreterPath),
                        TaskValidationSeverity.Error,
                        isBlocking: true,
                        detailIsResourceKey: false));
                }
    ...
    private async Task<bool> IsFileReferenceAvailableAsync(string value, CancellationToken ct)
    {
        try
        {
            var normalized = NormalizeRemotePath(value);
            ...
            if (!normalized.StartsWith("/", StringComparison.Ordinal) && !normalized.Contains("/", StringComparison.Ordinal))
                return await IsExecutableReferenceAvailableAsync(normalized, ct);

            var escaped = EscapeShellArg(normalized);
            var (_, _, exitCode) = await _ssh.ExecuteAsync($"test -f {escaped}", ct);
            return exitCode == 0;
        }
        catch
        {
    ...
    private static bool IsOptionalSbatchGresSynchronized(string script, string? gpuCount)
    {
        var normalizedGpu = (gpuCount ?? string.Empty).Trim();
        if (string.IsNullOrWhiteSpace(normalizedGpu))
            return !TryReadDirective(script, "--gres", out _);

        return IsSbatchOptionSynchronized(script, "--gres", $"gpu:{normalizedGpu}");
    }
    ...
    private static bool HasCommandPayload(CommandValidationItem command)
        => !string.IsNullOrWhiteSpace(command.CommandLine)
           ...
           || command.UsePythonInterpreter
           || !string.IsNullOrWhiteSpace(command.PythonInterpreterPath)
